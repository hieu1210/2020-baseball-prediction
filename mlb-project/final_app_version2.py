...
@st.cache
def loadRawData():
    df = pd.read_excel(open('MLB Games 2013-19.xlsx', 'rb'), sheet_name='Training')
...
#     return [gamesWonPercentage, totalRunsScored, totalRunsAllowed,
#             totalAtBats, totalHits, total2B, total3B, totalHR, totalRBI, totalBB,
#             totalk, OBP, SLG, ERA]
...
def getAnnualTeamData2(df, teamName, year):
    ...
    # total putouts
    totalPO = gamesHome['putouts_home'].sum()
    totalPO += gamesAway['putouts_away'].sum()
    ...
    # total Balks
    totalBK = gamesHome['BK_home'].sum()
    totalBK += gamesAway['BK_away'].sum()
    ...
    #total SB
    totalSB = gamesHome['SB_home'].sum()
    totalSB += gamesAway['SB_away'].sum()
    
    #total CS
    totalCS = gamesHome['CS_home'].sum()
    totalCS += gamesAway['CS_away'].sum()
    
    #total TB
    totalTB = gamesHome['TB_home'].sum()
    totalTB += gamesAway['TB_away'].sum()
    
    #total GIDP
    totalGDP = gamesHome['GIDP_home'].sum()
    totalGDP += gamesAway['GIDP_away'].sum()
    
    #total LOB
    totalLOB = gamesHome['LOB_home'].sum()
    totalLOB += gamesAway['LOB_away'].sum()
    
    #total A
    totalA = gamesHome['assists_home'].sum()
    totalA += gamesAway['assists_away'].sum()
    ...
    # total DP
    totalDP = gamesHome['DP_home'].sum()
    totalDP += gamesAway['DP_away'].sum()
    
    # total indER
    totalindER = gamesHome['indER_home'].sum()
    totalindER += gamesAway['indER_away'].sum()  
    
    # total WP
    totalWP = gamesHome['WP_home'].sum()
    totalWP += gamesAway['WP_away'].sum()  
    
    #Batting stats
    gamesWonPercentage = numGamesWon / numGames
    RunsPerGame = totalRunsScored / numGames
    battingAverage = totalH / totalAB
    ...
    #Pitching stats
    ERA = (totalteamER / (totalPO / 3)) * 9
    
    return [round(RunsPerGame,2), totalAB, totalH, total2B, total3B, totalHR, totalRBI, totalSB, totalCS, totalBB, totalSO, 
            round(battingAverage,3), round(OBP,3), round(SLG,3), round(OPS, 3), totalTB, totalGDP, totalHBP, totalSH, totalSF, totalIBB, totalLOB,
            round(RunsAllowedPerGame,2), round(DefEff,3), totalPO, totalA, totalE, totalDP, 
            round(gamesWonPercentage,3), totalteamER, totalBK, totalWP, round(ERA,2)]

def createAnnualDict2(df, year):
    annualDictionary = collections.defaultdict(list)
    teamList = df['Away_team_full'].unique()
    for team in teamList:
        team_vector = getAnnualTeamData2(df, team, year)
        annualDictionary[team] = team_vector
    return annualDictionary

def getAnnualTeamData3(df, teamName, year):
    annual_data = df[df['year'] == year]
...
def createAnnualDict3(df, year):
    ...
    for team in teamList:
        result_vector = getAnnualTeamData3(df, team, year)
...
def createGamePrediction(new_team1_vector, new_team2_vector, X, y):
    ...
    predict_team = lm.predict(diff)
    predictions = lm.predict_proba(diff)
    return predict_team, predictions

def formulatePredictions(df, test_data, X, y):
    ...
    for index, row in test_data.iterrows():
        ...
        prediction = createGamePrediction(team1_vector, team2_vector, X, y)
...
def formulatePredictions2019(df, test_2020_rev, X, y):
...
def formulatePredictions2018(df, test_2021, X, y):
...
def formulatePredictions2017(df, test_2020_org, X, y):
...
def main():
    st.title("Baseball Game Predictor")
    ...
    if st.checkbox("Getting A Team Stat by Year"):
        ...
        year = st.number_input("Enter the Year: ", 2013, 2021, key = "041195")
        if st.button("Get", key="0245"):
            result_df = getAnnualTeamData3(MLB_data, teamName, year)
            st.write(result_df)

    if st.checkbox("Getting All Team Stats by Year"):
        year = st.number_input("Enter the Year: ", 2013, 2021, key = "041196")
        if st.button("Get", key="0246"):
            result_df = createAnnualDict3(MLB_data, year)
            st.write(result_df)
    ...
    choose_testing = st.sidebar.selectbox("Choose the Testing Method", ["None", "2021 Season"])
    ...
    #     outcome = formulatePredictions(MLB_data, test_full, X, y)
    ...
    #     test_2020_rev = pd.read_excel(open('MLB Games 2020.xlsx', 'rb'), sheet_name='Full Rev')
    #     test_2020_rev['Date'] = pd.to_datetime(test_2020_rev['Date'], utc = False)
    #     outcome = formulatePredictions2019(MLB_data, test_2020_rev, X, y)
    ...
    #     tally = tally.sort_values(by=['conference', 'W'], ascending=[True, False])
    #     st.write(tally)
    #     draft.to_csv('predicted_2020_test.csv')
            
    if (choose_testing == '2021 Season'):
        ...
        outcome = formulatePredictions2018(MLB_data, test_2021, X, y)
        ...
        tally = tally.sort_values(by=['conference', 'W'], ascending=[True, False])
        st.write(tally)
        # draft.to_csv('predicted_2021.csv')
            
    # if (choose_testing == 'Full Original 2020 Season'):
    #     test_2020_org = pd.read_excel(open('MLB Games 2020.xlsx', 'rb'), sheet_name='Full Old')
    #     test_2020_org['Date'] = pd.to_datetime(test_2020_org['Date'], utc = False)
    #     outcome = formulatePredictions2017(MLB_data, test_2020_org, X, y)
    ...
    #     tally = tally.sort_values(by=['conference', 'W'], ascending=[True, False])
    #     st.write(tally)
    #     draft.to_csv('predicted_2020_org.csv')
